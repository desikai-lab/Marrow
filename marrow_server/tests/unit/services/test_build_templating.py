import pytest
from tools.builds import BuildManifest, OutputConfig, StepConfig, VersionConfig
from tools.build_processors import BuildContext, AppendTextProcessor, IncludeArtifactProcessor
...
def test_include_artifact_processor_cross_project_step_routes_to_correct_project(monkeypatch):
    ...
    step = StepConfig(action="include_artifact", path="some.md", project="CrossProjectXYZ", skip_reference=True)
    processor.process(step, ctx)
