...
from services.maintenance_service import MaintenanceService, MaintenanceReport
...
class TestMaintenanceService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_version_cleanup_fails_other_phases_still_execute(self):
        ...
        self.assertTrue(report.files_compacted)
        self.assertTrue(any("Version cleanup failed" in err for err in report.errors), f"Errors: {report.errors}")
