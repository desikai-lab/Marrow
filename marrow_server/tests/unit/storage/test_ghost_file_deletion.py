...
async def test_delete_skeleton_command_service_delegates_to_repository(mock_join, mock_repo_class, service):
...
async def test_delete_file_chunks_existing_path_removes_correct_records(mock_get_table):
    ...
    mock_table.search.return_value.where.return_value.limit.return_value.to_list.return_value = [1, 2, 3]
