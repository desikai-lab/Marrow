...
def sync_initial(project: str, dry_run: bool = False, force: bool = False) -> MigrationReport:
    ...
    file_map = {"active": "open", "paused": "paused", "done": "closed"}
            ...
            try:
                ...
                if not task_key:
                    report.errors.append(
                        {"task_title": task_data.get("title"), "error": "Missing task ID"}
                    )
                    continue
                ...
                record = TaskRecord(
                    ...
                    if isinstance(task_data.get("blocked_by"), list)
                    else [],
                    where=task_data.get("where", [])
                    if isinstance(task_data.get("where"), list)
                    else [],
                ...
                report.errors.append(
                    {
                        ...
                        "error": f"Transform/Load error: {str(e)}",
                    }
                )
