    ...
    async def __call__(self, scope, receive, send):
        ...
        async def custom_send(message):
            if message["type"] == "http.response.start":
                ...
                if is_sse:
                    headers = [(k, v) for k, v in headers if k.lower() not in (b"x-accel-buffering", b"cache-control", b"mcp-protocol-version")]
                ...
                if body and b"event: endpoint" in body and b"data: /messages" in body:
                    ...
                    message["body"] = body
                    debug_log(f">>> [SSE REWRITE] Forced absolute URL: {body.decode('utf-8', 'ignore').strip()}")
                elif body:
...
class TokenAuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        debug_log(f">>> [AUTH DEBUG] Path: {path}, Method: {request.method if 'method' in scope else '?'}")
        
        SKIP_PATHS = {"/.well-known", "/authorize", "/token", "/register", "/authorize_confirm", "/health", "/userinfo"}
        ...
        if token != SECRET_TOKEN:
            ...
            debug_log(f"    ProvidedToken: {repr(token)[:20]}..., Expected: {repr(SECRET_TOKEN)[:20]}...")
            ...
            response.headers["WWW-Authenticate"] = f'Bearer realm="mcp", resource_metadata="{metadata_url}"'
