...
class RegexMatchSpec(Specification):
    ...
    def is_satisfied_by(self, content: str) -> bool:
        ...
        if not match:
            self._error = (
                self.custom_error
                or f"Validation failed: Entry for regex '{self.regex}' not found in {self.step_path}"
            )
            return False
        ...
        if self.actual != self.expected:
            self._error = (
                self.custom_error
                or f"Validation failed for {self.step_path}. Expected '{self.expected}', but got '{self.actual}'"
            )
            return False
        return True
...
class RegexFindallSpec(Specification):
    ...
    def is_satisfied_by(self, content: str) -> bool:
        ...
        if not matches:
            self._error = (
                self.custom_error
                or f"Validation failed: No matches for regex '{self.regex}' in {self.step_path}"
            )
            return False
    ...
    def process(self, step: StepConfig, context: BuildContext):
        ...
        if step.section_name:
            ...
            if not content:
                raise ValueError(
                    step.error_message
                    or f"Validation failed: Section '{step.section_name}' not found in {step.path}"
                )

        spec = (
            RegexFindallSpec(step.regex, step.expected, step.path, step.error_message)
            if step.use_findall
            else RegexMatchSpec(step.regex, step.expected, step.path, step.error_message)
        )

        if not spec.is_satisfied_by(content):
            if context.verbose:
                print(
                    f"[VERBOSE] Validation for {step.path}:\n  Regex: {step.regex}\n  Expected: '{step.expected}'\n  Actual: '{spec.actual}'"
                )
