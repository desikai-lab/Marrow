    ...
    for i, line in enumerate(lines, start_line):
        ...
        if max_chars is not None and max_chars > 0:
            ...
            if len(current_combined) >= max_chars:
                final_text = current_combined[:max_chars] + f"\n... [Text truncated: limit {max_chars} characters exceeded]"
                if skip_chars > 0:
                    final_text = f"[Text truncated: {skip_chars} characters skipped at the beginning]\n... " + final_text
                return final_text
            
    final_combined = "\n".join(res_lines)
    if skip_chars > 0:
        final_combined = f"[Text truncated: {skip_chars} characters skipped at the beginning]\n... " + final_combined
...
class ReadStrategy(ABC):
    @abstractmethod
    def read(self, path: str, **kwargs) -> str: pass
...
class SaveStrategy(ABC):
    @abstractmethod
    def save(self, path: str, content: str, **kwargs) -> str: pass
    
    @abstractmethod
    def validate(self, content: str, **kwargs): pass
...
class AppendSectionStrategy(SaveStrategy):
    ...
    def save(self, path: str, content: str, **kwargs) -> str:
        ...
        return f"Section '{clean_section_name(kwargs.get('section_name'))}' appended to end of file."
...
class ReplaceChunkStrategy(SaveStrategy):
    ...
    def save(self, path: str, content: str, **kwargs) -> str:
        ...
        with open(path, "w", encoding="utf-8-sig", newline="") as f:
            f.write(final_content)
        return f"Line range {kwargs.get('start_line')}-{kwargs.get('end_line')} successfully replaced."
