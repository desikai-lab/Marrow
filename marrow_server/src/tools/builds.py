...
def parse_manifest(project: str, build_name: str) -> BuildManifest:
    ...
    try:
        ...
        if manifest.schema_version != MANIFEST_SCHEMA_VERSION:
            logger.warning(
                f"Manifest '{build_name}' uses schema_version {manifest.schema_version}, but system expects {MANIFEST_SCHEMA_VERSION}. Backward compatibility is active."
            )
...
def prepare_release_directory(project: str, manifest: BuildManifest, context) -> str:
    ...
    release_dir_name = raw_dir_name.replace("{version}", str(manifest.version)).replace(
        "{name}", manifest.name
    )
...
def run_project_build_logic(
    project: str, build_name: str, verbose: bool = False, variables: dict[str, str] | None = None
) -> BuildResult:
    """
    ...
    try:
        ...
        if manifest.output.format == "single_file":
            ...
            filename = raw_filename.replace("{version}", str(manifest.version)).replace(
                "{name}", manifest.name
            )
            from tools.utils.template_renderer import TemplateRenderer
        ...
        return BuildResult(
            success=False, output_path=None, steps_run=0, warnings=[], error=error_msg
        )
