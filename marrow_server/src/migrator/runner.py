...
def run_migrations_for_project(project_name: str, dry_run: bool = False) -> MigrationReport:
    ...
    report = MigrationReport(
        ...
        ending_version=starting,
    )

    pending = sorted(
        (m for m in LOCAL_STORAGE_LAYOUT_REGISTRY if m.from_version >= current),
    ...
    for migration in pending:
        if migration.from_version != current:
            ...
            _logger.error(
                "[migrator] %s: registry gap at version %d, stopping", project_name, current
            )
            report.errors.append(
                {"subsystem": _SUBSYSTEM, "version": current, "error": "registry gap"}
            )
            break
        try:
            ...
            if not dry_run:
                ...
                version_store.save_project_meta(
                    project_name, meta
                )  # persist after EACH step (ADR-0046 Decision 4, crash safety)
        except Exception as e:
            _logger.error(
                "[migrator] %s: subsystem=%s from=%d to=%d failed: %s",
                project_name,
                _SUBSYSTEM,
