...
class ReindexCommand(BaseCommand):
    ...
    def register_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--target", choices=["tasks", "artifacts", "both"], default="both", help="Reindexing target")
    ...
    def _reindex_artifacts(self, project_name: str, project_root: str, dry_run: bool):
        ...
        for root_dir, dirs, files in os.walk(project_root):
            ...
            for f in files:
                if f.endswith(".md"):
                    if any(x in f.lower() for x in ["backlog_active", "backlog_paused", "backlog_done", "backlog_index"]):
                        continue
        ...
        for file_path in tqdm(all_files, desc="Artifacts", unit="file"):
            try:
                ...
                if not dry_run:
                    ...
                    vector = embeddings_manager.generate_vector(clean_content, model_name=EMBEDDING_MODEL_TEXT)
