...
class TestArtifactPipeline:
    ...
    async def test_validation_handler_fail(self):
        ...
        mock_artifact_strategy_factory.get_save_strategy.side_effect = lambda m: AsyncMock()
        
        # Patching necessary components for the test scope
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr("tools.utils.filesystem_utils.validate_artifact_path", mock_validate_artifact_path)
            mp.setattr("tools.utils.artifact_strategies.ArtifactStrategyFactory", mock_artifact_strategy_factory)
    ...
    async def test_persist_handler_with_integrity_hook(self):
        ...
        mock_ctx.grouped_updates = {
            "/mock/path/file.md": [
                (0, {"path": "/mock/path/file.md", "mode": "replace_file", "content": "old_data"})
            ]
        }
        ...
        mock_hook = AsyncMock()
        mock_hook.validate_and_repair.return_value = "repaired_content" # Hook changes content
        ...
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr("tools.utils.filesystem_utils.validate_artifact_path", mock_validate_artifact_path)
            mp.setattr("tools.utils.filesystem_utils.create_artifact_backup", mock_create_artifact_backup)
            mp.setattr("tools.utils.artifact_strategies.ArtifactStrategyFactory", mock_strategy_factory)
            mp.setattr("tools.utils.artifact_integrity_hooks.ArtifactIntegrityRegistry", mock_registry)
            mp.setattr("tools.utils.storage.uow.UnitOfWork", lambda p: mock_uow) # Mock UOW access

            # Execute the logic using the patched/mocked dependencies
            from marrow_server.src.tools.artifact_pipeline import PersistHandler, ArtifactStrategyFactory # Need correct import path
            ...
            # Assertions
            mock_validate_artifact_path.assert_called_with("marrow", "/mock/path/file.md")
            mock_create_artifact_backup.assert_called_with("marrow", "/mock/path/file.md")
