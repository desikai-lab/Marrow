...
def setup_logging(log_file: str = "logs/worker.log", level: int = logging.INFO) -> None:
    ...
    logging.info("Worker logging initialized. File target: %s | Level: %s", log_file, logging.getLevelName(level))

async def extract_and_encode(filepath: str, encoder: Optional[LazyEncoder] = None):
    """
...
async def process_file(filepath: str, api_client: MCPClient, encoder: Optional[LazyEncoder] = None):
    """
...
async def serve(repo_dir: str, target_url: str, extensions: list[str], project_name: str, secret_token: str, run_init: bool = False):
    logger = logging.getLogger("worker")
    ...
    flush_task = asyncio.create_task(
        outbox.background_flush_loop(deliver_fn)
    )
...
def main():
    ...
    parser.add_argument("--repo-dir",      type=str, default=os.getcwd(),           help="Absolute or relative path to the codebase to monitor")
    parser.add_argument("--project-name",  type=str, default="DefaultProject",      help="Project identifier for the remote MCP server (e.g. 'YourProject')")
    parser.add_argument("--target-url",    type=str, default="http://localhost:8000",help="Root URL for the remote Vectorizer Transport Server")
    parser.add_argument("--extensions",    type=str, default=".cs,.ts,.py",         help="Comma-separated list of file extensions")
    parser.add_argument("--secret-token",  type=str, default=os.getenv("MCP_SECRET_TOKEN", ""),
